...
```bash
pwd
# Expected: /workspaces/NodeJS_PWA_Programming_For_The_Web_Task_Source
```
...
4. **Connect to Node.js**: "In our Node.js app, we're using HTTP for development. Why might that be okay?"
...
- "Can you find the port number our Node.js server is using? Where do you look?"
...
2. **Relate to Node.js project**:
   - "In our project, what files run in the browser?" (HTML, CSS, JS in public folder)
   - "What files run on the server?" (index.js, Node.js code)
3. **Interactive investigation**:
   - "Open browser DevTools → Sources tab"
   - "Which files do you see? Where are they from?"
   - "Now look at the terminal running Node.js → What do you see?"
4. **Build understanding**: "Let's trace a request from click to response"

#### Understanding Check Questions

- "If you change a colour in CSS, what needs to reload?"
- "If you change a route in index.js, what needs to restart?"
...
1. **Conceptual foundation**:
   ...
   - "What happens to variables when the Node.js server restarts?"
2. **SQL as a language**:
...
Let's verify your setup:
    ...
    node index.js  # What error message do you see?

💭 **Understanding Check**:
- "What does the error message say?"
- "Have you installed the dependencies? (`npm list`)"

📖 **Documentation**: See README.md - Section 'Setup your environment'

💡 **Learning Opportunity**:
This error teaches us about Node.js modules and dependencies. Express and sqlite3 need to be installed before Node.js can import them.

🎯 **Guided Steps**:
1. Read the error message carefully - what is it telling you?
2. Check if dependencies are installed: `ls node_modules`
3. If the folder is empty or missing modules, what command installs Node.js packages?
...
3. Check your registration code in app.js
   - Is the path correct?
   - Is it inside a `load` event listener?

⚠️ **Common Pitfalls**:
- Wrong file path in `register('js/serviceworker.js')` vs `register('/serviceworker.js')`
...
```text
📚 **Learning Context**:
You're learning about server-side routing, which is how Express decides what to show when someone visits different URLs.
...
🎯 **Guided Discovery**:
1. Look at this example in index.js:
...
- `res.sendFile()` sends an HTML file to the browser

📖 **Next Steps**: See README.md - Section 'Express Routing Basics'

⚠️ **Watch Out For**:
- Forgetting to use absolute paths with `path.join(__dirname, ...)`
- Using the same route path twice
- Not restarting Node.js after changes to index.js
```
...
3. Node.js queries the database using sqlite3
4. Database returns data
5. Node.js sends data to frontend (as JSON or in HTML)
6. Browser displays data to user

**Understanding Check**:
- "What happens if the database is empty?"
- "Where does the data come from in step 3?"
- "Who sees the final result?"
...
**Step 3 - Use the Data**:
Consider:
- What format is the data in? (rows array)
- How do we send it to the frontend?
- What does the frontend need to display it?

📖 **Resources**:
- SQL basics: README.md - Section 'Database Integration'
- Node.js sqlite3 module: Look at index.js - See how db.all() is used

💡 **Learning by Doing**:
Instead of copying code, try this:
1. Write the SQL query first (test it separately in SQLite viewer)
2. Then add the Node.js connection code
3. Finally, integrate with your Express route

This way you understand each piece!

⚠️ **Common Mistakes**:
- Forgetting that database operations are asynchronous (use callbacks)
- Not handling errors in the callback function
- SQL injection (never use string concatenation for queries! Use ? placeholders)
```
...
- **W3C standards**: Accessibility, internationalisation, validation
...
- What does the Node.js terminal show?
